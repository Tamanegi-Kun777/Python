...
def eval_expr(tree, depth=0):
    ...
    # func_callを追加！
        ...
        return run_func(func_name, args, depth=depth+1)
    
    elif tree.data in ('expr', 'term', 'atom'):
        return eval_expr(tree.children[0], depth)
        #print("condition:", eval_expr(children[0]))
def run_if(node, local_vars, depth):
    print("vars_array:", vars_array) # ← 確認用！
    print("node:", node)              # ← 追加！
    print("node.children:", node.children)  # ← 追加！
    children = node.children
    i = 0
    while i < len(children):
        print("children[i]:", children[i])  # ← 追加！
        print("i:", i)  # ← 追加！
        if i + 1 < len(children) and hasattr(children[i+1], 'data') and children[i+1].data == 'func_body':
            condition = eval_expr(children[i])
            if condition:
                for inner in children[i+1].children:
                    if hasattr(inner, 'data') and inner.data == 'return_stat':
                        return eval_expr(inner.children[0], depth)
                i += 2
            else:
                i += 2
        else:
            # 全条件falseの時のelse！
            if hasattr(children[i], 'data') and children[i].data == 'func_body':
                for inner in children[i].children:
    ...
    for stat in body:
        ...
        if hasattr(stat, 'data') and stat.data == 'func_body':
            for node in stat.children:
                ...
                # Treeで来た時！
                elif hasattr(node, 'data') and node.data == 'assign':
        ...
        if isinstance(stat, dict) and "assign" in stat:
            ...
            for node in stat.children:
                ...
                if isinstance(node,dict) and 'assign' in node:
                    ...
                    return eval_expr(node.children[0], depth)
                    ...
                    print("if_stat発見！")  # ← これは表示される？
                    children = node.children
                    i = 0
                    print("condition:", eval_expr(children[i]))  # ← ここに追加！
                    while i < len(children):
                        child = children[i]
                        
                        if i + 1 < len(children) and hasattr(children[i+1], 'data') and children[i+1].data == 'func_body':
                        #elif node.data == 'if_stat':
                        #    condition = eval_expr(node.children[0])
                        #    if condition:
                            condition = eval_expr(children[i])
                            if condition:
                                for inner in children[i+1].children:
                            ...
                            # ↑func_body以外が来た時！
                            if hasattr(children[i], 'data'):
                                # returnの処理！
                                if children[i].data == 'return_stat':
                                    return eval_expr(children[i].children[0], depth)
                                # assignの処理！
                                elif children[i].data == 'assign':
                                    var_name = str(children[i].children[0])
                                    value = eval_expr(children[i].children[1])
                                    if var_name not in vars_array:
                                        vars_array[var_name] = []
                                    vars_array[var_name].append(value)
                                for inner in children[i].children:
...
# クラス定義
    #__default__を追加して全部のノードを確認！
    #def __default__(self, data, children, meta):
    #    print("node:", data)
    #    return Tree(data, children)  # ← TreeとTokenをimportすればOK！
#    def sub(self, items):
#        left = items[0] if isinstance(items[0], int) else eval_expr(items[0])
#        right = items[1] if isinstance(items[1], int) else eval_expr(items[1])
#        return left - right
#   def add(self, items):
#        left = items[0] if isinstance(items[0], int) else eval_expr(items[0])
#        right = items[1] if isinstance(items[1], int) else eval_expr(items[1])
...
#        return left * right
    ...
    def class_def(self, items):
        print("class_def items:", items)
        global current_class
        name = str(items[0])
        body = items[1]  # ← class_bodyが返したdict！
        methods = {}
        vars = {}
        # bodyがdictの時！
        ...
        # bodyがTreeの時！
            ...
            print(body.pretty())  # ← bodyの中身を確認！
            for item in body.children:
                if hasattr(item, 'data') and item.data == 'func_def':
                    func_name = str(item.children[0])
                    params = item.children[1]
                    body_list = list(item.children[2:])
                    param_list = [str(p) for p in params.children] if hasattr(params, 'children') else []
                    methods[func_name] = {"params": param_list, "body": body_list}
    ...
    def method_call(self, items):
        ...
        # bodyを取り出す！
            ...
            for stat in body:
                ...
                elif hasattr(stat, 'data') and stat.data == 'func_body':
                    for node in stat.children:
                        if not hasattr(node, 'data'):
                            continue  # ← Treeじゃない時はスキップ！
            ...
            #  if_statの処理！
                            ...
                            result = run_if(node, vars_array, depth)
